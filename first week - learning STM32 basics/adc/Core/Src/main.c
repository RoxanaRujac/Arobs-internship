...
volatile uint32_t adc_value = 0;    // Current ADC value
...
int main(void)
{
  ...
  while (1)
  {
      ...
      {
          adc_value = HAL_ADC_GetValue(&hadc3);           // Get ADC value
          adc_sum += adc_value;                           // Add to sum
      }
      HAL_Delay(100); // Delay to reduce ADC read frequency
  }
}
